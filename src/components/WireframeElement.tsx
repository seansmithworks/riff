...
function Button({
  ...
  if (variant === "secondary") {
    return (
      <button
        type="button"
        className="w-full rounded-full border border-zinc-300 px-4 py-2 text-center text-[15px] font-medium text-zinc-700"
      >
        {label}
      </button>
    );
  ...
  return (
    <button
      type="button"
      className="w-full rounded-full bg-[#1F7A4D] px-4 py-2 text-center text-[15px] font-semibold text-white"
    >
      {label}
    </button>
  );
...
export function WireframeElement({ element }: { element: Element }) {
  switch (element.type) {
          ...
          {element.actions?.length ? (
            <div className="flex gap-2">
              {element.actions.map((action) => (
                <span key={action} className="text-[11px] text-zinc-500">
                  {action}
                </span>
              ))}
            </div>
    ...
    case "heading":
      return (
        <h3 className="px-3 text-[19px] font-bold text-zinc-900">
          {element.text}
        </h3>
      );
    ...
    case "input":
      return (
        <div className="flex flex-col gap-1 px-3">
          ...
          <div className="rounded-md border border-zinc-300 bg-white px-2.5 py-2 text-[13px] text-zinc-400">
            {element.placeholder ?? ""}
          </div>
        </div>
      );
    ...
    case "avatar":
      return (
        <div className="flex items-center gap-2 px-3">
          ...
          {element.name ? (
            <span className="text-[13px] text-zinc-600">{element.name}</span>
          ) : null}
        </div>
      );
